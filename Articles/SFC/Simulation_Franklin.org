...
def model(
  ...
  calculated_gih = phi_0 - phi_1*((1+rm)/(1+infla) - 1)
  """
  ...
  model.var('g_Z', desc='Autonomous growth rate', default = calculated_gih)
  model.var('g_C', desc='Autonomous consumption growth rate', default = calculated_gih)
  ...
  model.var('V_h', desc='Household net nominal wealth', default = 1000)
  # model.var('V_hf', desc='Household net financial wealth', default = 1000)
  # model.var('V_hf_aux', desc='Household net financial wealth (aux)')
  model.var('V_hr', desc='Household net real wealth', default = 1000)
  model.var('V_f', desc='Firms net wealth', default = 500)
  model.var('V_b', desc='Banks net wealth', default = 0)
  ...
  model.param('gC', desc='Autonomous consumption growth rate', default = calculated_gih)
  model.param('equal_rates', desc='Flag', default = equal_rates)
  ...
  model.add('g_C = (equal_rates)*g_Z + (1 - equal_rates)*gC')

  model.add('YDk = FD + rm*M_h(-1) - rmo*MO(-1)')
  model.add('Ck = (1+g_C)*Ck(-1)')
  ...
  # model.add('d(V_hf) = -Ck')
  model.add('V_h =  M_h - MO  + K_HD*ph') # Eq 15
  model.add('V_hr = M_h - MO  + K_HD') # Eq 15
  ...
  model.add('DT = MO')
  model.add('DN = DT - M_h')
  # model.add('V_hf_aux = M_h - MO') # Residual
...
*** Residential investment growth rate higher than autonomous consumption


#+BEGIN_SRC python :results output latex table
base = model(equal_rates = 0)
df = SolveSFC(base, time=1000)
shock = ShockModel(base_model=base, create_function=model(), variable='phi_0', increase=0.01, time = 1000)
df_gih=shock
clock_plots(shock = shock, filename = 'Clock_higher_gIh_referee.png', variable='g_Z')
plot_shock(shock = shock, filename = 'Shock_higher_gIh_referee.png', df=df)
plot_norms(shock = shock, filename = 'Shock_higher_gIh_Norms_referee.png', df=df)
other_plots(shock, df=df)

shock_Ih_Ck = shock.round(decimals = 5).tail(1).transpose().loc['alpha':,:]
shock_Ih_Ck.columns = ['$g_{I_h} > g_C$']
print(shock_Ih_Ck.to_latex())
#+END_SRC
...
infla       &     0.000000e+00 \\
gC          &     2.400000e-02 \\
equal\_rates &     0.000000e+00 \\
\_K\_f\_\_1     &     4.646478e+27 \\
\_M\_h\_\_1     &     6.492989e+28 \\
\_MO\_\_1      &     6.368165e+28 \\
\_Ck\_\_1      &     8.652071e+22 \\
\_Lf\_\_1      &     1.248243e+27 \\
\_h\_\_1       &     5.100000e-02 \\
\_L\_\_1       &     1.248243e+27 \\
\_M\_\_1       &     6.492989e+28 \\
\_K\_HD\_\_1    &     6.368165e+28 \\
\_I\_h\_\_1     &     2.093981e+27 \\
...
*** Residential investment growth rate lower than autonomous consumption

#+BEGIN_SRC python :results output latex table
base = model(equal_rates = 0)
df = SolveSFC(base, time=1000)
shock = ShockModel(base_model=base, create_function=model(), variable='gC', increase=0.01, time = 1000)
df_ck=shock
clock_plots(shock = shock, filename = 'Clock_higher_gC_referee.png', variable='g_Z')
plot_shock(shock = shock, filename = 'Shock_higher_gC_referee.png', df=df)
plot_norms(shock = shock, filename = 'Shock_higher_gC_Norms_referee.png', df=df)
# other_plots(shock, df=df)

shock_Ck_Ih = shock.round(decimals = 5).tail(1).transpose().loc['alpha':,:]
shock_Ck_Ih.columns = ['$g_{I_h} < g_C$']
print(shock_Ck_Ih.to_latex())
#+END_SRC
...
gC          &     3.400000e-02 \\
equal\_rates &     0.000000e+00 \\
\_K\_f\_\_1     &     3.159745e+27 \\
\_M\_h\_\_1     &     8.542212e+26 \\
\_MO\_\_1      &     5.428750e+24 \\
\_Ck\_\_1      &     1.423907e+27 \\
\_Lf\_\_1      &     8.487924e+26 \\
\_h\_\_1       &     5.100000e-02 \\
\_L\_\_1       &     8.487924e+26 \\
\_M\_\_1       &     8.542212e+26 \\
...
#+BEGIN_SRC python :results graphics file :file ./figs/Compared_Shocks_1_Zs.png :cache no
base = model(equal_rates = 0)
df = SolveSFC(base, time=1000)
df["Z/Y"] = df["Z"]/df["Y"]
df_base = df
...
df_gih['Y'].pct_change().plot(ls ='-',lw=3,
                           #color = "black",
                           color = "tab:blue",
                           label = "$g_{I_h} > g_{Ck}$", ax = ax[0,0]
)
df_ck['Y'].pct_change().plot(ls ='-', lw=3,
                           #color = "darkgray",
                           color = "tab:red",
                           label = "$g_{I_h} < g_{Ck}$", ax = ax[0,0])
...
df_gih['Z/Y'].plot(ls ='-', lw=3,
                #color = "black",
                color = "tab:blue",
                label = "$g_{I_h} > g_{Ck}$", ax = ax[0,1])
df_ck['Z/Y'].plot(ls ='-', lw=3,
                #color = "darkgray",
                color = "tab:red",
                label = "$g_{I_h} < g_{Ck}$", ax = ax[0,1])
...
df_gih['u'].plot(ls ='-', lw=3,
              #color = "black",
              color = "tab:blue",
              label = "$g_{I_h} > g_{Ck}$", ax = ax[1,0])
df_ck['u'].plot(ls ='-', lw=3,
              #color = "darkgray",
              color = "tab:red",
              label = "$g_{I_h} < g_{Ck}$", ax = ax[1,0])
...
df_gih['h'].plot(ls ='-', lw=3,
              #color = "black",
              color = "tab:blue",
              label = "$g_{I_h} > g_{Ck}$", ax = ax[1,1])
df_ck['h'].plot(ls ='-', lw=3,
              #color = "darkgray",
              color = "tab:red",
              label = "$g_{I_h} < g_{Ck}$", ax = ax[1,1])
...
#plt.show()
fig.savefig("./figs/Compared_Shocks_1_Zs.png", dpi = 300)
#+END_SRC

#+RESULTS:
[[file:./figs/Compared_Shocks_1_Zs.png]]



**** Figure 2

#+BEGIN_SRC python :results graphics file :file ./figs/Compared_Shocks_2_Zs.png :cache no
base = model()
df = SolveSFC(base, time=1000)
df["Z/Y"] = df["Z"]/df["Y"]
df_base = df

df_gih["TIME"] = [i+1 for i in range(len(df_gih.index))]
df_ck["TIME"] = [i+1 for i in range(len(df_ck.index))]

fig, ax = plt.subplots(2,2, figsize=(19.20,10.80))

sns.scatterplot(y = 'Z/Y', x='u', data=df_gih, size="TIME", sizes = (1,100), color = 'tab:blue', legend=False, ax=ax[0,0])
sns.scatterplot(y = 'Z/Y', x='u', data=df_ck, size="TIME", sizes = (1,100), color = 'tab:red', legend=False, ax=ax[0,0])

sns.lineplot(y = 'Z/Y', x='u', data=df_gih, sort=False, color = 'tab:blue', ax=ax[0,0])
sns.lineplot(y = 'Z/Y', x='u', data=df_ck, sort=False, color = 'tab:red', ax=ax[0,0])
ax[0,0].set_title('A Share of NCC autonomous expenditures and capacity utilization\n(Dots size grow in time)')

df_gih['K_k'].plot(ls ='-', lw=3, color = "tab:blue", label = "$g_{I_{h}} > g_{Ck}$", ax = ax[0,1])
df_ck['K_k'].plot(ls ='-', lw=3, color = "tab:red", label = "$g_{I_{h}} < g_{Ck}$", ax = ax[0,1])
ax[0,1].axhline(y = df_base['K_k'].iloc[-1], ls ='--', lw=1.5, color = "black", label = "Baseline")
ax[0,1].ticklabel_format(useOffset=False)
ax[0,1].set_title('B Houses share on\nReal Assets ($K_k$)')

((df_gih["MO"][1:]*df_gih["rm"][2:])/df_gih['YDk'][2:]).plot(ls ='-', lw=3, color = "tab:blue", label = "$g_{I_{h}} > g_{Ck}$", ax = ax[1,0])
((df_ck["MO"][1:]*df_ck["rm"][2:])/df_ck['YDk'][2:]).plot(ls ='-', lw=3, color = "tab:red", label = "$g_{I_{h}} < g_{Ck}$", ax = ax[1,0])
ax[1,0].axhline(y = ((df_base["MO"].iloc[-1]*df_base["rm"].iloc[-1])/df_base['YDk'].iloc[-1]), ls ='--', lw=1.5, color = "black", label = "Baseline")
ax[1,0].ticklabel_format(useOffset=False)
ax[1,0].set_title('C Capitalist Indebtedness\n(as % Capitalist disposable income)')

(df_gih['Fn']/df_gih['K_f']).plot(ls ='-', lw=3, color = "tab:blue", label = "$g_{I_{h}} > g_{Ck}$", ax = ax[1,1])
(df_ck['Fn']/df_ck['K_f']).plot(ls ='-', lw=3, color = "tab:red", label = "$g_{I_{h}} < g_{Ck}$", ax = ax[1,1])
...
#plt.show()
fig.savefig("./figs/Compared_Shocks_2_Zs.png", dpi = 600)
#+END_SRC

#+RESULTS:
[[file:./figs/Compared_Shocks_2_Zs.png]]
...
** Residential investment and autonomous consumption growing at same rate
...
infla       &     0.000000e+00 \\
gC          &     2.400000e-02 \\
equal\_rates &     1.000000e+00 \\
\_K\_f\_\_1     &     6.062022e+25 \\
\_M\_h\_\_1     &     5.945535e+26 \\
\_MO\_\_1      &     5.860454e+26 \\
\_Ck\_\_1      &     1.123111e+25 \\
\_Lf\_\_1      &     8.508095e+24 \\
\_h\_\_1       &     4.350000e-02 \\
\_L\_\_1       &     8.508095e+24 \\
\_M\_\_1       &     5.945535e+26 \\
...
gC          &     2.400000e-02 \\
equal\_rates &     1.000000e+00 \\
\_K\_f\_\_1     &     4.534949e+23 \\
\_M\_h\_\_1     &     5.403019e+24 \\
\_MO\_\_1      &     5.428750e+24 \\
\_Ck\_\_1      &     8.652071e+22 \\
\_Lf\_\_1      &    -2.573088e+22 \\
\_h\_\_1       &     3.600000e-02 \\
\_L\_\_1       &    -2.573088e+22 \\
\_M\_\_1       &     5.403019e+24 \\
\_K\_HD\_\_1    &     5.428750e+24 \\
\_I\_h\_\_1     &     1.272363e+23 \\
\_ph\_\_1      &     1.000000e+00 \\
TIME        &     1.051000e+03 \\
...
gC          &  2.400000e-02 \\
equal\_rates &  1.000000e+00 \\
\_K\_f\_\_1     &  3.600342e+23 \\
\_M\_h\_\_1     &  4.277937e+24 \\
\_MO\_\_1      &  4.297398e+24 \\
\_Ck\_\_1      &  6.779290e+22 \\
\_Lf\_\_1      & -1.946113e+22 \\
\_h\_\_1       &  3.600000e-02 \\
\_L\_\_1       & -1.946113e+22 \\
\_M\_\_1       &  4.277937e+24 \\
...
gC          &  2.400000e-02 \\
equal\_rates &  1.000000e+00 \\
\_K\_f\_\_1     &  5.035507e+25 \\
\_M\_h\_\_1     &  4.969998e+26 \\
\_MO\_\_1      &  4.902237e+26 \\
\_Ck\_\_1      &  9.334785e+24 \\
\_Lf\_\_1      &  6.776130e+24 \\
\_h\_\_1       &  4.300000e-02 \\
\_L\_\_1       &  6.776130e+24 \\
\_M\_\_1       &  4.969998e+26 \\
