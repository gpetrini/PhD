...
def model(
  ...
  model.add('d(V_hf) = -Ck')
  model.add('V_h =  V_hf  + K_HD*ph') # Eq 15
  model.add('V_hr = V_hf  + K_HD') # Eq 15
...
((df_gih["DN"][1:]*df_gih["rm"][2:])/df_gih['K_HD'][2:]).plot(ls ='-', lw=3, color = "tab:blue", label = "$g_{I_{h}} > g_{Ck}$", ax = ax[1,0])
((df_ck["DN"][1:]*df_ck["rm"][2:])/df_ck['K_HD'][2:]).plot(ls ='-', lw=3, color = "tab:red", label = "$g_{I_{h}} < g_{Ck}$", ax = ax[1,0])
