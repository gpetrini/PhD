...
#+PROPERTY: header-args:python :resul ts output drawer replace :session *VECM* :exports none :tangle ./code/VECM.py :eval never-export
...
df["Residential Investment growth rate"] = df["Residential Investment"].pct_change()
...
This movement supports the ordinary view of the Great Recession as originated by a subprime crisis parencite:mian_consequences_2009.
The top 1%, however, also increased their wealth in form of housing, as well as their mortgage debts, corroborating to a "New narrative" of the crisis parencite:albanesi_2017_Credit.
As discussed before, this housing boom was possible because of some changes of housing market regulation and housing finance parencite:federal_deposit_insurance_corporation_savings_1997,mccarthyMonetaryPolicyTransmission2002,wall_too_2010.
...
More recently, textcite:petrini_2021_TD simulated a Stock-Flow Consistent model calibrated for the US economic data in which the rate of growth of the residential investment is governed by houses own-rate of interest.
...
Then, we employ conventional unit root tests (Table ref:unitroot in appendix ref:appen:A) as well as textcite:johansen_estimation_1991 procedure to assess whether houses own-rate of interest and residential investment growth rate share a common long-run trend (Table ref:Johansen in appendix ref:appen:A).
...
In addition to the usual information criteria[fn::For the purpose of determining the order of the model, we use four infomation criteria. One (BIC) indicates no lag, the other three (AIC, FPE and HQIC) indicate four lags (Table ref:criterios in appendix ref:appen:A).], there are economic arguments in favor of using lags.
...
# We report opposites results since houses own-rate of interest are statistically significant and this is the case for a broad timespan (Table ref:tab:robust in Appendix ref:appen:A).
