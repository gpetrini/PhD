...
/home/gpetrini/.local/lib/python3.8/site-packages/scipy/stats/morestats.py:1371: RuntimeWarning: invalid value encountered in greater_equal
  pos = x >= 0  # binary mask
/tmp/babel-WhNddm/python-eXqgtX:37: SettingWithCopyWarning:
...
Traceback (most recent call last):
  ...
  File "/tmp/pyxvpfJN", line 3, in <module>
  File "/tmp/babel-WhNddm/python-GbaX56", line 2, in <module>
    df[["Inflation", "gIh", "Own Interest rate", "Interest rate"]].to_csv(
...
:results:
  ...
  File "/home/gpetrini/.local/lib/python3.8/site-packages/statsmodels/tsa/vector_ar/vecm.py", line 1815, in <listcomp>
    caused_ind = [get_index(self.names, c) for c in caused]
...
ValueError: 'd_gIh' is not in list
...
Based on the recent US housing boom episode, textcite:teixeira_crescimento_2015  proposed the houses own rate of interest ($own$).
...
textcite:teixeira_crescimento_2015 also uses this procedure to describe residential investment growth rate ($g_{I_{h}}$, see Figure ref:propria_investo).
...
\end{equation}
#+end_export
where $\phi_0$ stands for long-term determinants (/e.g./ demographic factors, housing and credit policies, etc.) while $\phi_1$ captures the price-interest rate sensibility of demand for houses.
...
During a housing bubble period (/e.g./ 2002-5 in the US), it is real estate inflation that governs houses own rate of interest.
...
More recently, textcite:petrini_2021_TD simulated a Stock-Flow Consistent model for the US with the already mentioned houses own rate of interest.
...
Our sample period (1992:Q1 to 2019:Q1) starts after institutional changes due to the Savings and Loans crisis of the 1980s (see Table ref:structbreak in appendix ref:appen:A for structural break tests)[fn::This institutional changes --- notably Financial Institutions Reform, Recovery, and Enforcement Act (FIRREA) in 1989 and Federal Deposit Insurance Corporation Improvement Act  (FDICIA) in 1991 --- increased the credit volume to households. As a consequence, real estate finance has increased considerably in the following periods. For a discussion about the Savings and Loans crisis, see texcite:federal_deposit_insurance_corporation_savings_1997.].
...
This dynamic could be captured by its *expected* value.
...
Additionally, if houses own rate of interest depicts residential investment growth rate as textcite:teixeira_crescimento_2015 describes, its lagged coefficients ($\gamma_{2}$) should be negative and statistically significant (hypothesis 6).
