...
\end{equation}
#+end_export
where $\phi_0$ includes factors which vary more slowly than house prices (/e.g./ demographic factors, housing and credit policies, etc.) while $\phi_1$ captures the price-interest rate sensibility of demand for houses.
...
For the US case, textcite:fair_macroeconometric_2018 concludes that the nominal mortgage rate is more adequate since inflation variables "picks up" not included variables (such as expectations).
