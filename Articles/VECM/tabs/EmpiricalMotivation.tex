% \begin{landscape}
\begin{table}[H]
    \caption{Residential investment determinants in econometric models}
...
% \begin{tabular}{p{0.1\textwidth}p{0.1\textwidth}p{0.1\textwidth}p{0.7\textwidth}}
    ...
    \textcite{arestis_residential_2015} & 17 OECD countries (1970-2013) & ARDL Model & \makecell{$RHP(+), RDY(-), LRINT(-),$\\$ Cr(+), UN(-)$}\\\hline
    \textcite{kohlscheen_2018_Residential} & 15 OECD countries (1970-2017) & OLS & \makecell{$Q(+), RHP(+), GDPc(+),$\\$ POP(+), NM(+), H(-)$} \\\hline
    \textcite{fair_macroeconometric_2018} & US (1954-2007 and 195-2015) & FIML and 3SOLS & \makecell{$AGs(\pm), RDY(+), H(-),$\\$ NMRA(-), NHW(+)$} \\\hline
    \hline
    \end{tabularx}
    % \end{tabular}
    % } %end resize
