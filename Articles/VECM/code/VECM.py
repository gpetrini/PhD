...
fig, ax = plt.subplots(
    2, 3, sharex=True, sharey=True, squeeze=False, figsize=(19.2, 10.8)
)
...
fig.text(0.5, 0.03, "Capacity utilization (Total industry)", ha="center", fontsize=20)
fig.text(
    -0.01,
    0.5,
...
plt.suptitle("(Markers sizes increases over time)")
...
def testes_coint(
    ...
    for i in range(1, maxlag):
        print(50 * "=")
        cointegracao(
            ts0=series.iloc[:, 0], ts1=series.iloc[:, 1:], signif=signif, lag=i
        )
        print("\nTESTE DE JOHANSEN\n")
        print("Teste SEM constante")
        result = select_coint_rank(
            endog=series, k_ar_diff=i, det_order=-1, signif=signif
        )  ## Warning: 1
        print(result.summary())
        print(f"Para lag = {i} e significância = {signif*100}%, Rank = {result.rank}")
        print("\nTeste COM constante\n")
        result = select_coint_rank(
            endog=series, k_ar_diff=i, det_order=0, signif=signif
        )  ## Warning: 1
        print(result.summary())
        print(f"Para lag = {i} e significância = {signif*100}%, Rank = {result.rank}")
...
def LjungBox_Pierce(resid, signif=0.05, boxpierce=False, k=4):
    ...
    for i in range(var):
        print(
            "Testing for ",
            resid.columns[i].upper(),
            ". Considering a significance level of",
            signif * 100,
            "%",
        )
        result = acorr_ljungbox(x=resid.iloc[:, i - 1], lags=k, boxpierce=boxpierce)[
            i - 1
        ]
...
def analise_residuos(results, nmax=15):
    ...
    for serie in residuals.columns:
        ...
        plot_acf(
            residuals[serie], ax=ax[0], title=f"ACF Resíduo de {serie}", zero=False
        )
        plot_pacf(
            residuals[serie], ax=ax[1], title=f"PACF Resíduo de {serie}", zero=False
        )
...
def plot_lags(results=results, trimestres=[2, 5]):
    ...
    for i in range(len(trimestres)):
        sns.regplot(
            y=df[series[0]],
            x=df[series[1]].shift(-trimestres[i]),
            color="black",
            ax=ax[i, 0],
            order=2,
        )
        ...
        sns.regplot(
            x=df[series[0]].shift(-trimestres[i]),
            y=df[series[1]],
            color="black",
            ax=ax[i, 1],
            order=2,
...
fig.savefig(
    "./figs/FEVD_VECMpython_TxPropria.png",
    dpi=300,
    bbox_inches="tight",
    pad_inches=0.2,
    transparent=False,
...
for serie in series:
    ...
    plt.close("all")
    sns.scatterplot(y=residuals[serie], x=residuals[serie].shift(-1), color="darkred")
    sns.despine()
