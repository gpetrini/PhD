...
Our series are co-integrated at 5$\backslash$% significance level which allows us to estimate a error correction model and evaluate the previous hypothesis \cite{enders_applied_2014}.
...
The short-term relationship between \(own\) and \(g_{Ih}\) (\(\beta_{1, is}\) coefficients) are not statistically significant at 5$\backslash$%\footnote{The expected result (7) can also be validated from the inspection of Table \ref{Estimacao} in which only the fourth lag of own interest rate equation is statistically significant.}.
In addition, coefficients \(\gamma_{2,s}\) are negative and statistically significant at 5$\backslash$%, supporting hypothesis 6 (see Table  \ref{Estimacao}). 
...
In addition, we find that such contribution is greater than 50$\backslash$% beyond the third quarter.
...
\label{sec:orgf2d9257}
No potential conflict of interest was reported by the authors.

\section*{References}
\label{sec:org13b3757}
\printbibliography[heading=none]


\appendix
\section{Statistical Appendix}
\label{sec:orgdb67843}
\label{appen:A}
