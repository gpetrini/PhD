...
df <-  map_dfr(
  ...
  mutate(crise = case_when(
           (date >= ymd(begin[1]) %m+% years(-2) & date <= ymd(begin[1]) %m+% years(2)) ~ begin[1],
           (date >= ymd(begin[2]) %m+% years(-2) & date <= ymd(begin[2]) %m+% years(2)) ~ begin[2],
           (date >= ymd(begin[3]) %m+% years(-2) & date <= ymd(begin[3]) %m+% years(2)) ~ begin[3],
           (date >= ymd(begin[4]) %m+% years(-2) & date <= ymd(begin[4]) %m+% years(2)) ~ begin[4],
           (date >= ymd(begin[5]) %m+% years(-2) & date <= ymd(begin[5]) %m+% years(2)) ~ begin[5],
           (date >= ymd(begin[6]) %m+% years(-2) & date <= ymd(begin[6]) %m+% years(2)) ~ begin[6],
           (date >= ymd(begin[7]) %m+% years(-2) & date <= ymd(begin[7]) %m+% years(2)) ~ begin[7],
           (date >= ymd(begin[8]) %m+% years(-2) & date <= ymd(begin[8]) %m+% years(2)) ~ begin[8],
           (date >= ymd(begin[9]) %m+% years(-2) & date <= ymd(begin[9]) %m+% years(2)) ~ begin[9],
           (date >= ymd(begin[10]) %m+% years(-2) & date <= ymd(begin[10]) %m+% years(2)) ~ begin[10]
         )) %>%
  mutate(crise = crise %>% as.character() %>% as.factor()) %>%
  pivot_longer(cols = -c(date, crise, recessions), names_to="Series", values_to = "value") %>%
  ## mutate(Series = as.factor(Series)) %>%
  filter(date>=ymd('1980-01-01') & date <=ymd('2011-01-01')) %>%
  drop_na(crise)
...
df <-  map_dfr(
  ...
  select(date, recessions, GDP, Residential_Investment, NonResidential_Investment, DurableGoods) %>%
  mutate(crise = case_when(
           (date >= ymd(end[1]) %m+% years(-2) & date <= ymd(end[1]) %m+% years(2)) ~ end[1],
           (date >= ymd(end[2]) %m+% years(-2) & date <= ymd(end[2]) %m+% years(2)) ~ end[2],
           (date >= ymd(end[3]) %m+% years(-2) & date <= ymd(end[3]) %m+% years(2)) ~ end[3],
           (date >= ymd(end[4]) %m+% years(-2) & date <= ymd(end[4]) %m+% years(2)) ~ end[4],
           (date >= ymd(end[5]) %m+% years(-2) & date <= ymd(end[5]) %m+% years(2)) ~ end[5],
           (date >= ymd(end[6]) %m+% years(-2) & date <= ymd(end[6]) %m+% years(2)) ~ end[6],
           (date >= ymd(end[7]) %m+% years(-2) & date <= ymd(end[7]) %m+% years(2)) ~ end[7],
           (date >= ymd(end[8]) %m+% years(-2) & date <= ymd(end[8]) %m+% years(2)) ~ end[8],
           (date >= ymd(end[9]) %m+% years(-2) & date <= ymd(end[9]) %m+% years(2)) ~ end[9],
           (date >= ymd(end[10]) %m+% years(-2) & date <= ymd(end[10]) %m+% years(2)) ~ end[10]
         )) %>%
  mutate(crise = crise %>% as.character() %>% as.factor()) %>%
  pivot_longer(cols = -c(date, crise, recessions), names_to="Series", values_to = "value") %>%
  ## mutate(Series = as.factor(Series)) %>%
...
In a letter to President Roosevelt, in the mid of the Great Depression, Keynes present the case of housing as the best aid to fight the crisis and to foster economic recovery \cite[p.~436]{keynes_collected_1978}.
...
Perhaps, this lack of attention could be attributed to residential investment small share of aggregate demand (see figure ref:fig:share).
...
An aspect that macroeconomists are paying attention is the role of residential investment to business cycles.
Even before the Great Recession, textcite:green_follow_1997 analyses the relation between residential investment, non-residential investment and GDP for the US from 1952 to 1992.
His conclusion is that residential investment leads the cycle, while non-residential investment (i.e., firms business investment) lags the cycle.
However he argues it does not imply a causal relationship.
He restricts its relevance as temporal precedence indicator.
This result would be due to forward-looking behavior of households.

textcite:leamer_housing_2007 is more incisive. He states that housing is the business cycle.
...
Based on a neoclassical business cycle framework, they conclude that long-term nominal mortgage rate is a relevant transmission channel from interest rates to housing costs.
More recently, textcite:huang_is_2018 assess both \citeauthor*{leamer_housing_2007}'s \citeyear{leamer_housing_2007} hypotheses related to residential investment (prediction and causality) for 17 countries.
...
In each panel, we present the growth rate of selected expenditures 2 years before and after recession begin[fn::This similar reasoning can be found in textcite:leamer_housing_2007.].
In most of them, residential investment growth rate is negative before the recession start.
The Great Recession presents is a remarkable example of this pattern while dot-com crisis is an exception.
...
In a heterodox perspective, textcites:fiebiger_semi-autonomous_2018,fiebiger_trend_2017 present residential investment as a Luxemburg-type of external market providing a source of autonomous demand to absorb economic surplus.
...
Recently, textcite:perez_Montiel_2021 test econometrically the assertion that this kind of expenditure leads not only the businesses cycle but also the long-run economic growth for the US economy. Their research finds evidence of this claim, without finding any relevant feedback from output to residential investment.
