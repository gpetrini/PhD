...
#These behave the same way built in True, False, and None behave
class Contradiction(object):
    def __call__(self):
        raise TypeError, "'Contradiction' object is not callable"
...
class Remainder(object):
    def __call__(self):
        raise TypeError, "'Remainder' object is not callable"
...
class Impossible(object):
    def __call__(self):
        raise TypeError, "'Impossible' object is not callable"
...
##################################################################
# Transformation of interval/ratio scale variable into fuzzy set
#
...
##################################################################
# classes and other module-level functions (e.g., functions that are
# shared by multiple classes and/or need to be accessible at the
# module level)
...
class QcaDataset(Dataset):
    ...
    A QcaDataset is a dataset that is suitable for QCA analysis.  One
    column is designated as the "outcome" column; all others, as
    "causal conditions."  Aside from the header rows, all cell values
    must range between 0.0 and 1.0, inclusive.

    Convert a QcaDataset instance into a truth table by passing it to
    the .from_dataset() method of a TruthTableFactory instance."""

    def __init__(self, indata, outcome_col=None, include=[], exclude=[]):
        # indata can be a file-like object or a Dataset-like object
        # (i.e., a list of lists)
        try:
            dsf = DatasetFactory()
            list.__init__(self, dsf.from_any(indata))
        except TypeError:
            list.__init__(self, indata)

        # which column is the outcome?  let's try to do the work for
        # the user.  if outcome_col isn't specfied, assume that the
        # last column is outcome; if alphanumeric passed in, search
        # column names; if numeric passed in, use that value as column
        if outcome_col is None:
            outcome_col = len(self[0])-1
        else:
            if not(str(outcome_col).isdigit()):
                try:
                    outcome_col = self[0].index(outcome_col)
                except ValueError:
                    raise QcaError, 'Outcome condition not found: %s' % outcome_col
            else:
                outcome_col = int(outcome_col)
        try:
            outcome = self[0][outcome_col]
        except IndexError:
            raise QcaError, 'Outcome column does not exist: %s' % outcome_col

        # by default, include all other columns as causal conditions
        causal_conds = self[0][:]  # slice so that we don't change original data
        del causal_conds[outcome_col]
        del causal_conds[0]

        if include:
            i = 0
            while i < len(include):
                if include[i].isdigit():
                    include[i] = causal_conds[int(include[i])-1]
                # sanity checks
                if include[i] == outcome:
                    raise QcaError, "Included condition '%s' is the outcome" % include[i]
                if include[i] not in causal_conds:
                    raise QcaError, "Included condition '%s' is not a causal condition" % include[i]
                i += 1
        ...
        if exclude:
            ...
            while i < len(exclude):
                if exclude[i].isdigit():
                    exclude[i] = causal_conds[int(exclude[i])-1]
                # sanity checks
                if exclude[i] == outcome:
                    raise QcaError, "Excluded condition '%s' is the outcome" % exclude[i]
                if exclude[i] not in causal_conds:
                    raise QcaError, "Excluded condition '%s' is not a causal condition" % exclude[i]
                i += 1

        if include:  # include might be out of order
            causal_conds = [cc for cc in causal_conds if cc in include]

        # drop any excluded conditions
        tokeep = [self[0].index(cc) for cc in causal_conds if cc not in exclude]
        tokeep.append(0)  # cases
        tokeep.append(outcome_col)
        tokeep.sort()  # resort columns according to order in original dataset

        working = []
        for row in self:
            working.append([row[idx] for idx in tokeep])

        # outcome column may have changed
        outcome_col = working[0].index(outcome)
        self.outcome_col = outcome_col

        # validate that all data is fuzzy
        for i,row in enumerate(working):
            if i > 0:  # initial row should be variable names
                for j,col in enumerate(row):
                    if j > 0:  # initial col should be observation names
                        try:
                            working[i][j]=float(col)
                            if working[i][j] < 0.0 or working[i][j] > 1.0:
                                raise BoundaryError
                        except BoundaryError:
                            msg='Data values must be between 0.0 and 1.0: %s (%s, %s)' % (working[i][j], working[i][0], working[0][j])
                            raise BoundaryError, msg
                        except ValueError:
                            msg='Non-numeric value in dataset: %s (%s, %s)' % (working[i][j], working[i][0], working[0][j])
                            raise ValueError, msg

        # and reassign to self
        self[:] = working[:]

    def outcome(self):
        """Return name of outcome."""
        return self[0][int(self.outcome_col)]

    def outcome_memberships(self):
        """Return outcome membership scores."""
        return [obs[int(self.outcome_col)] for obs in self[1:]]

    def causal_conds(self):
        """Return names of causal conditions as a list."""
        cc = self[0][:]  # slice so that we don't chage original data
        del cc[self.outcome_col]
        del cc[0]
        return cc

    def causal_memberships(self):
        """Return causal membership scores as a list of list."""
        cms = []
        for obs in self[1:]:
            obs = obs[:]  # don't change original data
            del obs[self.outcome_col]
            del obs[0]
            cms.append(obs)
        return cms

    def isnec(self, causal_cond, consist_thresh):
        """Test if causal condition is consistent with necessity."""
        outcome_memberships = self.outcome_memberships()
        cond_indices = []
        for cond in causal_cond:
            cond_indices.append(self.causal_conds().index(cond))

        memberships = []
        for row in self.causal_memberships():
            membership = 0.0
            for index in cond_indices:
                membership = max(membership, row[index])
            memberships.append(membership)

        return consist_nec(memberships, outcome_memberships) >= float(consist_thresh)
