...
to setup
  ...
  if density-ratio = 0 [set density-ratio random 100] ;; just to initialize and create at least one house
  if initial-house-price = 0 [set initial-house-price random 10] ;; just to initialize
  if Locality = 0 [set Locality random 10] ;; just to initialize
...
to update-house-price
  ask houses [
    ifelse shocked? = false [
      ...
      let unshocked-houses houses with [shocked? = false]
      set price (mean [price] of agents-around-here)
      set color scale-color brown price (max [price] of unshocked-houses + 1 ) (min [price] of unshocked-houses - 1 )
        ]
...
#+begin_src xml :exports none :results none
  <experiment name="baseline-run" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
