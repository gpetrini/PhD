...
experiments = pd.DataFrame.from_dict(doc["experiments"])
experiment = experiments['experiment']

exps = []

for i in range(len(experiment)):
    base_var = experiment.iloc[i]
    exp_names = base_var['@name']
    base_df = pd.DataFrame.from_dict(base_var['enumeratedValueSet'])
    variables = base_df['@variable']
    vals = base_df['value']

    for v in range(len(variables)):
        tmp = dict(vals[v]['@value'])
        values = pd.DataFrame(tmp)
...
#+begin_src R :results graphics file :file ./figs/baseline_house_price_mean.png
df %>%
...
#+begin_src R :results graphics file :file ./figs/densityxlocality_house_price_mean.png
df %>%
  ggplot(aes(x = time, y = `House price mean`, group = round(time / 50))) +
