...
#+PROPERTY:header-args netlogo :results output drawer :eval never-export :session cluster-geo :exports code
...
The idea of this toy model is to evaluate how house prices diffusion spread across a *clusted grid* and affects the house price index.
...
** Clusterization
:PROPERTIES:
  :header-args: :noweb-ref src-geo-cluster
  :END:
