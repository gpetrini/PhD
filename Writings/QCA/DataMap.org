...
ggplot(df) +
  ...
  facet_wrap(~ country) +
  xlab("") + ylab("Mortgage share on total loans for private sector") +
  geom_hline(yintercept=.50, linetype="dashed", color = "black", size=0.25) +
  scale_x_date(
    breaks = seq.Date(min(df$year), lubridate::ymd(2020,truncated=2L), by="30 years"),
    date_minor_breaks = "10 years", date_labels = "%Y")

ggsave(
  './figs/Jorda_Facet.png',
  width = 16, height = 10, dpi = 150, units = "in", device='png'
       )
#+end_src
...
** Facet plot (Mortgage/Loans to Household)

#+begin_src R :results graphics file :file ./figs/Jorda_FacetHH.png
df = read_dta(
  'http://www.macrohistory.net/JST/JSTdatasetR4.dta',
  encoding = NULL,
  col_select = NULL,
  ...
  n_max = Inf,
  .name_repair = "unique"
) %>% mutate(Mortgage_TotalHH = tmort/thh) %>%
  mutate(year = lubridate::ymd(year, truncated = 2L)) %>%
  select(year, country, Mortgage_TotalHH) %>%
  pivot_wider(names_from = country, values_from = Mortgage_TotalHH) %>%
  rowwise() %>% mutate("[Average]" = mean(c_across(Australia:USA), na.rm=TRUE)) %>%
  pivot_longer(!year, names_to = "country", values_to = "Mortgage_TotalHH")

ggplot(df) +
  geom_line(aes(year, Mortgage_TotalHH, colour = country), color='black') +
  gghighlight(use_direct_label = FALSE) +
  facet_wrap(~ country) +
  xlab("") + ylab("Mortgage share on total loans to households") +
  geom_hline(yintercept=.50, linetype="dashed", color = "black", size=0.25) +
  scale_x_date(
    breaks = seq.Date(min(df$year), lubridate::ymd(2020,truncated=2L), by="30 years"),
    date_minor_breaks = "10 years", date_labels = "%Y")
...
ggsave(
  './figs/Jorda_FacetHH.png',
  width = 16, height = 10, dpi = 150, units = "in", device='png'
       )
#+end_src

#+RESULTS:
[[file:./figs/Jorda_FacetHH.png]]



** Grouped facet
#+begin_src R :results graphics file :file ./figs/Jorda_FacetGrouped.png
df <- read_dta(
  "http://www.macrohistory.net/JST/JSTdatasetR4.dta",
  encoding = NULL,
  col_select = NULL,
  ...
  pivot_wider(names_from = country, values_from = mortgaging) %>%
  rowwise() %>%
  mutate("[Average]" = mean(c_across(Australia:USA), na.rm = TRUE)) %>%
  pivot_longer(!year, names_to = "country", values_to = "mortgaging") %>%
  group_by(year) %>%
...
df <- df %>%
  ...
  mutate(Categories = case_when(
           country == "USA" | country == "UK" | country == "Ireland" | country == "Australia" ~ "Market oriented",
           country == "Germany" | country == "Austria" ~ "Central European corporatist",
           country == "[Average]" ~ "Average",
           country == "Portugal" | country == "Spain" | country == "Turkey" ~ "Southern European (Geographically)",
           country == "Denmark" | country == "Finland" | country == "Iceland" | country == "Norway" | country == "Sweden" |
           country == "Netherlands" | country == "France" | country == "Belgium" ~ "Welfare countries"
         )) %>%
  mutate(Categories = as.factor(replace_na(Categories, "Uncategorized"))) %>%
  group_by(Categories) %>%
  mutate(tmp = as.factor(country) %>% as.numeric() %>% as.factor()) %>%
  ungroup()


ggplot(df) +
  geom_line(aes(year, mortgaging, colour = tmp)) +
...
ggsave(
  './figs/Jorda_FacetGrouped.png',
  width = 16, height = 10, dpi = 150, units = "in", device='png'
       )
#+end_src

** Real estate share of bank lending
#+begin_src R :results pp table
df <- read_dta(
  "http://www.macrohistory.net/JST/JSTdatasetR4.dta",
  encoding = NULL,
  col_select = NULL,
...
|            |        |        |        |        |
|:-----------|-------:|-------:|-------:|-------:|
|year        | 1970.00| 1989.00| 2007.00| 2016.00|
|Sweden      |    0.81|    0.56|    0.60|    0.62|
|Denmark     |    0.59|    0.63|    0.60|    0.69|
|USA         |    0.55|    0.61|    0.70|    0.63|
|UK          |    0.51|    0.43|    0.52|    0.67|
|Switzerland |    0.49|    0.72|    0.87|    0.96|
|Netherlands |    0.46|    0.33|    0.55|    0.54|
|Finland     |    0.44|    0.33|    0.44|    0.46|
|Germany     |    0.42|    0.36|    0.51|    0.50|
|[Average]   |    0.38|    0.41|    0.55|    0.59|
|Norway      |    0.36|    0.41|    0.68|    0.73|
|France      |    0.33|    0.38|    0.49|    0.53|
|Australia   |    0.32|    0.30|    0.49|    0.57|
|Canada      |    0.30|    0.47|    0.51|    0.48|
|Belgium     |    0.30|    0.30|    0.45|    0.52|
|Italy       |    0.29|    0.32|    0.48|    0.60|
|Portugal    |    0.16|    0.25|    0.44|    0.49|
|Spain       |    0.12|    0.24|    0.58|    0.51|
|Japan       |    0.07|    0.25|    0.43|    0.47|
#+end_example
...
df <- read_dta(
  ...
  filter(year == 1930 | year == 1970 | year == 1989 | year == 2000 | year == 2007 | year == max(year)) %>%
  mutate(mortgaging = tmort / tloans) %>%
  ## mutate(year = lubridate::ymd(year, truncated = 2L)) %>%
  select(year, country, mortgaging) %>%
  pivot_wider(names_from = country, values_from = mortgaging) %>%
...
ggplot(df, aes(tidytext::reorder_within(country, mortgaging, year), mortgaging, fill = Categories)) +
  geom_bar(show.legend = TRUE, stat = "identity", color = "black") +
  facet_wrap(~year, scales = "free_y") +
  coord_flip() +
  scale_y_continuous(expand = c(0, 0)) +
  tidytext::scale_x_reordered() +
  xlab("") +
  ylab("Mortgage share on total loans for private sector") +
  geom_text(aes(label = round(mortgaging, 2)),
...
* WID dataset: Personal and Private Housing/Debt at local prices

** Facet plot
- Checar se tem Personal Housing, Private Housing, Private Debt

#+begin_src R
library(stringr)
df = read.csv(
  './raw/WID_Private_Personal_Housing_Debt_JordaCountries.csv',
  skip = 1,
  sep = ';'
)
cols = c("Year")
for(col in colnames(df)[-c(1)]){
